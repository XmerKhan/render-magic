...
import type { TimelineScene, EditSettings, AspectRatio, MotionGraphicsSettings } from '@/types';
import { MapPin, Calendar } from 'lucide-react';

const DEFAULT_MOTION_GRAPHICS: MotionGraphicsSettings = {
  lowerThird: true,
  callout: true,
  dateStamp: true,
  location: true,
...
const LowerThird: React.FC<{ scene: TimelineScene; aspectRatio: AspectRatio }> = ({ scene, aspectRatio }) => {
  ...
  return <div style={{ position: 'absolute', left: '6%', bottom, display: 'flex', flexDirection: 'column', gap: 4, opacity, transform: `translateX(${slideX}%)` }}>
    <div style={{ backgroundColor: 'rgba(251, 191, 36, 0.95)', padding: '0.8vh 2vh', borderRadius: '0.4vh', boxShadow: '0 4px 12px rgba(0,0,0,0.4)' }}>
      <p style={{ margin: 0, fontFamily: FONT_STACKS.inter, fontWeight: 700, fontSize, color: '#18181b', lineHeight: 1.2 }}>{scene.lowerThird.name}</p>
    </div>
    {scene.lowerThird.role && <p style={{ margin: 0, fontFamily: FONT_STACKS.inter, fontWeight: 500, fontSize: roleSize, color: '#e4e4e7', paddingLeft: '0.5vh', textShadow: '0 2px 4px rgba(0,0,0,0.6)' }}>{scene.lowerThird.role}</p>}
  </div>;
};
...
const TextCallout: React.FC<{ scene: TimelineScene; aspectRatio: AspectRatio }> = ({ scene, aspectRatio }) => {
  ...
  return <div style={{ position: 'absolute', top, left: '50%', transform: `translateX(-50%) scale(${scale})`, opacity, textAlign: 'center' }}>
    <div style={{ backgroundColor: 'rgba(0, 0, 0, 0.7)', padding: '1.5vh 3vh', borderRadius: '0.6vh', border: '2px solid rgba(251, 191, 36, 0.6)', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
      <p style={{ margin: 0, fontFamily: FONT_STACKS.inter, fontWeight: 800, fontSize, color: '#fbbf24', lineHeight: 1.1, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{scene.callout}</p>
    </div>
  </div>;
};
...
const DateStamp: React.FC<{ scene: TimelineScene; aspectRatio: AspectRatio }> = ({ scene, aspectRatio }) => {
  ...
  return <div style={{ position: 'absolute', top, left: '50%', transform: `translateX(-50%) translateY(${translateY}px)`, opacity, display: 'flex', alignItems: 'center', gap: '0.8vh', backgroundColor: 'rgba(0, 0, 0, 0.65)', padding: '0.6vh 1.5vh', borderRadius: '0.4vh' }}>
    <Calendar size={isPortrait ? 18 : 14} color="#fbbf24" />
    <p style={{ margin: 0, fontFamily: FONT_STACKS.inter, fontWeight: 600, fontSize, color: '#f4f4f5', letterSpacing: '0.08em' }}>{scene.date}</p>
  </div>;
};
...
const LocationPin: React.FC<{ scene: TimelineScene; aspectRatio: AspectRatio }> = ({ scene, aspectRatio }) => {
  ...
  return <div style={{ position: 'absolute', top, left: '50%', transform: 'translateX(-50%)', opacity, display: 'flex', alignItems: 'center', gap: '0.6vh', backgroundColor: 'rgba(0, 0, 0, 0.6)', padding: '0.5vh 1.2vh', borderRadius: '0.4vh' }}>
    <MapPin size={isPortrait ? 16 : 14} color="#4ade80" />
    <p style={{ margin: 0, fontFamily: FONT_STACKS.inter, fontWeight: 500, fontSize, color: '#d4d4d8' }}>{scene.location}</p>
  </div>;
};
...
const QuoteCard: React.FC<{ scene: TimelineScene; aspectRatio: AspectRatio }> = ({ scene, aspectRatio }) => {
  ...
  return <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity, transform: `translateY(${translateY}px)` }}>
    ...
    <div style={{ position: 'relative', maxWidth, textAlign: 'center', padding: '0 4vh' }}>
      <p style={{ margin: 0, fontFamily: FONT_STACKS.georgia, fontWeight: 400, fontSize, color: '#f4f4f5', lineHeight: 1.4, fontStyle: 'italic' }}>&ldquo;{scene.quote}&rdquo;</p>
      <div style={{ margin: '2vh auto 0', height: 2, width: '40%', background: 'linear-gradient(90deg, transparent, #fbbf24, transparent)' }} />
    </div>
  </div>;
};

export const MotionGraphics: React.FC<{ scene: TimelineScene; settings: EditSettings }> = ({ scene, settings }) => {
  const controls = { ...DEFAULT_MOTION_GRAPHICS, ...(settings.motionGraphics ?? {}) };
  return <Sequence from={0} durationInFrames={scene.durationFrames}>
    {controls.lowerThird && <LowerThird scene={scene} aspectRatio={settings.aspectRatio} />}
    {controls.callout && <TextCallout scene={scene} aspectRatio={settings.aspectRatio} />}
    {controls.dateStamp && <DateStamp scene={scene} aspectRatio={settings.aspectRatio} />}
    {controls.location && <LocationPin scene={scene} aspectRatio={settings.aspectRatio} />}
    {controls.quoteCard && <QuoteCard scene={scene} aspectRatio={settings.aspectRatio} />}
  </Sequence>;
};
