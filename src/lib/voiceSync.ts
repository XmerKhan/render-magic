...
function normalizeWord(value: string): string { return value.toLowerCase().replace(/[’']/g, '').replace(/[^a-z0-9]+/g, ''); }
function tokenize(value: string): string[] { return value.split(/\s+/).map(normalizeWord).filter(Boolean); }
...
function scoreSpan(scriptWords: string[], transcriptWords: string[], start: number, endExclusive: number): number {
  ...
  let bagScore = 0;
  for (const word of scriptWords) { let best = 0; for (const candidate of span) best = Math.max(best, similarity(word, candidate)); bagScore += best; }
  bagScore /= Math.max(1, scriptWords.length);
  return alignedScore * 0.7 + bagScore * 0.2 + lengthRatio * 0.1;
}

function findBestSpan(scriptWords: string[], transcriptWords: string[], cursor: number, maxStart: number, minimumWordsAfter: number): { start: number; endExclusive: number; score: number } {
  ...
  const maxLen = Math.min(transcriptWords.length - cursor - minimumWordsAfter, Math.max(minLen, Math.ceil(scriptWords.length * 1.45)));
  if (maxLen < minLen) return best;
  for (let start = cursor; start <= maxStart; start++) {
    if (scriptWords.length <= transcriptWords.length - start) {
      let exact = true;
      for (let i = 0; i < scriptWords.length; i++) if (scriptWords[i] !== transcriptWords[start + i]) { exact = false; break; }
...
export function alignScriptToTranscript(scriptLines: string[], transcript: TranscriptWord[], mediaIds: string[]): VoiceSyncResult {
  ...
  const words = transcript.filter((item) => Number.isFinite(item.startTime) && Number.isFinite(item.endTime) && item.endTime >= item.startTime).map((item) => ({ ...item, word: normalizeWord(item.word) })).filter((item) => item.word);
  ...
  for (let i = 0; i < cleanScript.length; i++) {
    ...
    if (match.endExclusive <= match.start || match.endExclusive > transcriptWords.length) throw new Error(`Unable to align script scene ${i + 1}. Check that the transcript contains the same narration words in the same order.`);
  ...
  const lines: VoiceSyncLine[] = matches.map((match, index) => {
    ...
    const startTime = index === 0 ? 0 : words[match.start]!.startTime;
    const endTime = next ? Math.max(startTime + 0.001, words[next.start]!.startTime) : Math.max(startTime + 0.001, words[match.endExclusive - 1]!.endTime);
    return { sceneId: `scene${index + 1}`, text: cleanScript[index]!, startTime, endTime, confidence: match.score };
  });

  const segments: ScriptSegment[] = lines.map((line, index) => ({ sceneId: line.sceneId, mediaId: mediaIds[index]!, startTime: line.startTime, endTime: line.endTime, text: line.text }));
...
export function parseOriginalScript(content: string): string[] {
  ...
  if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
    ...
    const raw = Array.isArray(parsed) ? parsed : parsed && typeof parsed === 'object' ? ((parsed as Record<string, unknown>).scenes ?? (parsed as Record<string, unknown>).segments ?? (parsed as Record<string, unknown>).lines ?? []) : [];
  ...
  const lines = trimmed.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => line.replace(/^(?:scene\s*)?\d+\s*[:.)-]\s*/i, '').trim()).filter(Boolean);
...
export function parseSceneOrder(content: string): string[] {
  ...
  if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
    const parsed = JSON.parse(trimmed) as unknown;
    const raw = Array.isArray(parsed) ? parsed : parsed && typeof parsed === 'object' ? ((parsed as Record<string, unknown>).scenes ?? (parsed as Record<string, unknown>).order ?? (parsed as Record<string, unknown>).media ?? []) : [];
...
export function parseTimestampedTranscript(content: string): TranscriptWord[] {
  ...
  if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
    const parsed = JSON.parse(trimmed) as unknown;
    const raw = Array.isArray(parsed) ? parsed : parsed && typeof parsed === 'object' ? ((parsed as Record<string, unknown>).words ?? (parsed as Record<string, unknown>).transcript ?? []) : [];
    if (!Array.isArray(raw)) throw new Error('Transcript JSON must contain a words array.');
    const result = raw.map((item) => { const value = item as Record<string, unknown>; const start = Number(value.startTime ?? value.start ?? value.start_sec ?? 0); const end = Number(value.endTime ?? value.end ?? value.end_sec ?? start); return { word: String(value.word ?? value.text ?? ''), startTime: start, endTime: end }; });
  ...
  for (const rawLine of trimmed.split(/\r?\n/)) {
    ...
    if (match) {
      const ms = (match[3] ?? '0').padEnd(3, '0').slice(0, 3), ems = (match[6] ?? '0').padEnd(3, '0').slice(0, 3);
      const start = Number(match[1]) * 60 + Number(match[2]) + Number(ms) / 1000, end = Number(match[4]) * 60 + Number(match[5]) + Number(ems) / 1000;
      const parts = match[7]!.trim().split(/\s+/).filter(Boolean), duration = Math.max(0, end - start);
      parts.forEach((word, index) => result.push({ word, startTime: start + duration * index / parts.length, endTime: start + duration * (index + 1) / parts.length })); continue;
    }
    const simple = line.match(/^(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s+(.+)$/);
    if (simple) {
      const start = Number(simple[1]), end = Number(simple[2]), parts = simple[3]!.trim().split(/\s+/).filter(Boolean), duration = Math.max(0, end - start);
