...
interface MediaBinProps {
  assets: MediaAsset[]; onAssetsChange: (assets: MediaAsset[]) => void;
  voiceoverFile: File | null; onVoiceoverChange: (file: File | null) => void;
  musicFile: File | null; onMusicChange: (file: File | null) => void;
  scriptFile: File | null; onScriptChange: (file: File | null) => void;
  originalScriptFile: File | null; onOriginalScriptChange: (file: File | null) => void;
  transcriptFile: File | null; onTranscriptChange: (file: File | null) => void;
  sceneOrderFile: File | null; onSceneOrderChange: (file: File | null) => void;
}
...
export function MediaBin({
  ...
  const mediaInputRef = useRef<HTMLInputElement>(null), audioInputRef = useRef<HTMLInputElement>(null);
  const musicInputRef = useRef<HTMLInputElement>(null), scriptInputRef = useRef<HTMLInputElement>(null);
  const originalScriptInputRef = useRef<HTMLInputElement>(null), transcriptInputRef = useRef<HTMLInputElement>(null);
  const sceneOrderInputRef = useRef<HTMLInputElement>(null);
  const [dragOver, setDragOver] = useState(false), [loading, setLoading] = useState(false);
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    if (newAssets.length) onAssetsChange([...assets, ...newAssets]);
    setLoading(false);
  ...
  const fileButton = (label: string, file: File | null, icon: React.ReactNode, onClick: () => void, hint: string, active: string) => (
    <button onClick={onClick} className={`w-full rounded-lg border p-2.5 text-left transition-colors ${file ? active : 'border-zinc-700 hover:border-zinc-600 bg-zinc-800/50'}`}>
      <div className="flex items-center gap-2">{icon}<div className="flex-1 min-w-0"><p className="text-xs text-zinc-300 font-medium truncate">{file ? file.name : label}</p><p className="text-[10px] text-zinc-500">{file ? 'Loaded' : hint}</p></div></div>
    </button>
  );
  ...
  return <div className="flex flex-col h-full bg-zinc-900 border-r border-zinc-800">
    <div className={`m-3 rounded-xl border-2 border-dashed p-4 text-center transition-colors cursor-pointer ${dragOver ? 'border-amber-500 bg-amber-500/10' : 'border-zinc-700 hover:border-zinc-600'}`} onClick={() => mediaInputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}>
      <Upload className="w-6 h-6 mx-auto mb-2 text-zinc-400" /><p className="text-sm text-zinc-300 font-medium">Drop media here</p><p className="text-xs text-zinc-500 mt-1">Images & videos — upload in Scene 1 → Scene 2 order</p>
    ...
    <div className="flex-1 overflow-y-auto px-3 pb-3 min-h-0">
      {assets.length > 0 && <><p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-2">Media Bin ({assets.length})</p><div className="grid grid-cols-2 gap-2">
      ...
      <div className="mt-4 space-y-2"><p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">Audio & Script</p>
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-2.5 text-[10px] text-zinc-500 leading-relaxed"><b className="text-zinc-400">Auto Sync:</b> Voiceover + original script + word timestamps → scene timing. Scene Order file optional; Media Bin order is used when it is not supplied. Legacy timestamp JSON/SRT remains supported.</div>
